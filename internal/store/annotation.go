...
func (sqlStore *SQLStore) DeleteClusterAnnotation(clusterID string, annotationName string) error {
	...
	// TODO: new method get all installationsIDs for cluster instead of full cluster annotation?
	...
	for _, ci := range clusterInstallations {
		...
		if model.ContainsAnnotation(annotations, annotation) {
			return fmt.Errorf(
				"cannot delete annotation '%s' from cluster '%s', it is used by one or more installations scheduled on the cluster",
				annotation.Name, clusterID,
			)
		}
	}
	...
	if rows != 1 {
		return fmt.Errorf("error deleting cluster annotation, expected 1 row to be affectedm was %d", rows)
	}
...
// TODO: remove returns from creates to be consistent
	...
	for _, ci := range clusterInstallations {
		...
		if !containsAllAnnotations(clusterAnnotations, annotations) {
			return nil, fmt.Errorf(
				"cannot add annotations to installation '%s', one or more clusters " +
					"on which installation is scheduled do not contain one or more new annotations",
				installationID)
		}
	}
...
func (sqlStore *SQLStore) DeleteInstallationAnnotation(installationID string, annotationName string) error {
	...
	if rows != 1 {
		return fmt.Errorf("error deleting installation annotation, expected 1 row to be affectedm was %d", rows)
	}
